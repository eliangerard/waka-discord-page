import './Card.css';
import Logo from '../assets/logo.png'
import { useRef, useEffect } from 'react';

export const Card = () => {
    const inputCode = useRef(null);
    const copiedToast = useRef(null);

    useEffect(() => {
        const urlParams = new URLSearchParams(window.location.search);
        const code = urlParams.get("code");
        inputCode.current.value = code;
    }, []);


    const handleCopy = () => {
        navigator.clipboard.writeText(inputCode.current.value);
        console.log(inputCode.current.value);
        copiedToast.current.style.opacity = "100%"
        setTimeout(() => {
            copiedToast.current.style.opacity = "0%";
        }, 3000);
    };

    return (
        <>
            <div className='card'>
                <img src={Logo} alt="" />
                <p>¡Aquí está tu código!</p>
                <div className='flex'>
                    <input ref={inputCode} type="text" value="..." disabled={true} />
                    <button onClick={handleCopy}>
                        <div ref={copiedToast}><p>¡Copiado!</p></div>
                        <svg width="800px" height="800px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path d="M16 12.9V17.1C16 20.6 14.6 22 11.1 22H6.9C3.4 22 2 20.6 2 17.1V12.9C2 9.4 3.4 8 6.9 8H11.1C14.6 8 16 9.4 16 12.9Z" />
                            <path d="M17.0998 2H12.8998C9.81668 2 8.37074 3.09409 8.06951 5.73901C8.00649 6.29235 8.46476 6.75 9.02167 6.75H11.0998C15.2998 6.75 17.2498 8.7 17.2498 12.9V14.9781C17.2498 15.535 17.7074 15.9933 18.2608 15.9303C20.9057 15.629 21.9998 14.1831 21.9998 11.1V6.9C21.9998 3.4 20.5998 2 17.0998 2Z" />
                        </svg>
                    </button>
                </div>
                <div className='flex padding'>
                    <p>Ahora, usa el comando <code>/code</code> y agrega el código de arriba para terminar de vincular tu cuenta</p>
                </div>
            </div>
        </>
    )
}